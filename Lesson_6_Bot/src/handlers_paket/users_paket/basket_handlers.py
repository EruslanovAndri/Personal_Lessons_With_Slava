...
from keyboards_paket import basket_keyboard,command_defoult_keyboard,basket_callback
from loader import dp,db,bot
from states_paket import BuyerState
from keyboards_paket.inlines_paket import basket_keyboard
...
async def del_basket(call: CallbackQuery):
    ...
    await bot.delete_message(chat_id=call.message.chat.id,message_id=call.message.message.id)
...
async def start_buy(call: CallbackQuery):
    ...
    await bot.call.message.answer(text='Укажите дату доставки.')
...
async def get_name(message: Message, state: FSMContext):
    user_basket = db.select_user_basket(id=message_from_user.id)[1]
    user_basket = [item_data.split(':') for item_data in user_basket.split()]
    ...
    text = f'Заявка' \
        ...
        f'\n\nТовар:'    
    for item_id,item_count in user_basket:
        _,name,_,_ = db.select_item_info(id=item_id)[0]
        text += f'\n{name}: {item_count}'
